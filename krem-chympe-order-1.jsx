...
// TODO: replace with the real payment details before going live.
// (These will move into the Super Admin settings panel in a later build.)
const BUSINESS = {
  upiId: "kremchympe@upi",
  payeeName: "Krem Chympe Waterfall",
  bank: {
    accountName: "Krem Chympe Waterfall",
...
export default function App() {
  ...
  const submitPayment = async (method, receiptDataUrl) => {
    if (!order) return;
    const fresh = await getShared("order:" + order.id, order);
    const updated = {
      ...fresh,
      ...
      statusHistory: [...(fresh.statusHistory || []), { status: "payment_pending", at: new Date().toISOString() }],
    };
    await setShared("order:" + order.id, updated);
    setOrder(updated);
  };
...
function TrackingView({ order, requestText, setRequestText, sendRequest, onNewOrder, onPaymentSubmit }) {
  const isCancelled = order.status === "cancelled";
  const isRejected = order.status === "rejected";
  const isChangesRequested = order.status === "changes_requested";
  const needsPayment = order.status === "accepted";
    ...
    <div className="max-w-md mx-auto pb-16 px-4 pt-5">
      <div className="text-center mb-5">
        ...
        <h2 className="font-display text-xl mt-1" style={{ color: "var(--ink)", fontWeight: 600 }}>
          {isCancelled ? "Order Cancelled" : isRejected ? "Order Declined" : needsPayment ? "Complete Payment" : "Tracking Your Order"}
        </h2>
      </div>

      {(isRejected || isCancelled || isChangesRequested) ? (
        <div style={{ background: "var(--card)", borderColor: isChangesRequested ? "var(--bamboo)" : "var(--danger)" }} className="rounded-xl border p-4 text-sm" >
          <p style={{ color: isChangesRequested ? "var(--bamboo)" : "var(--danger)" }} className="font-medium">
            {order.rejectReason || "The restaurant was unable to take this order."}
          </p>
        </div>
      ) : needsPayment ? (
        <PaymentPanel order={order} onSubmit={onPaymentSubmit} />
      ) : (
...
function CopyField({ label, value }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2 py-1.5">
      <div>
        <div className="text-[11px] uppercase tracking-wide opacity-60" style={{ color: "var(--ink)" }}>{label}</div>
        <div className="font-mono text-sm" style={{ color: "var(--ink)" }}>{value}</div>
      </div>
      <button onClick={copy} style={{ background: copied ? "var(--spring)" : "var(--paper)", color: copied ? "white" : "var(--ink)", borderColor: "var(--stone)" }} className="text-xs font-medium px-2.5 py-1 rounded-lg border shrink-0">
        {copied ? "Copied" : "Copy"}
      </button>
    </div>
  );
...
function PaymentPanel({ order, onSubmit }) {
  const [method, setMethod] = useState(null); // upi | qr | bank
  const [receipt, setReceipt] = useState(null);
  const [compressing, setCompressing] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const upiString = `upi://pay?pa=${encodeURIComponent(BUSINESS.upiId)}&pn=${encodeURIComponent(BUSINESS.payeeName)}&am=${order.total}&cu=INR&tn=${encodeURIComponent(order.id)}`;
  ...
  if (!method) {
    return (
      <div style={{ background: "var(--card)", borderColor: "var(--stone)" }} className="rounded-xl border p-4">
        <p className="text-sm mb-3" style={{ color: "var(--ink)" }}>The restaurant accepted your order. Choose how you'd like to pay <span className="font-mono font-semibold">₹{order.total}</span>.</p>
        <div className="space-y-2">
          <button onClick={() => setMethod("upi")} style={{ background: "var(--paper)", borderColor: "var(--stone)", color: "var(--ink)" }} className="w-full text-left rounded-xl border p-3 text-sm font-medium">Pay via UPI ID</button>
          <button onClick={() => setMethod("qr")} style={{ background: "var(--paper)", borderColor: "var(--stone)", color: "var(--ink)" }} className="w-full text-left rounded-xl border p-3 text-sm font-medium">Scan QR Code</button>
          <button onClick={() => setMethod("bank")} style={{ background: "var(--paper)", borderColor: "var(--stone)", color: "var(--ink)" }} className="w-full text-left rounded-xl border p-3 text-sm font-medium">Bank Transfer</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: "var(--card)", borderColor: "var(--stone)" }} className="rounded-xl border p-4">
      <button onClick={() => setMethod(null)} className="text-xs font-medium mb-3" style={{ color: "var(--moss-dark)" }}>← Choose a different method</button>

      {method === "upi" && (
        <div>
          <CopyField label="UPI ID" value={BUSINESS.upiId} />
          <CopyField label="Amount" value={"₹" + order.total} />
          <CopyField label="Reference" value={order.id} />
        </div>
      )}
      ...
      {method === "qr" && (
        <div className="flex flex-col items-center">
          <img
            src={`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(upiString)}`}
            alt="UPI payment QR code"
            width={200}
            height={200}
            style={{ background: "white", borderRadius: 12 }}
          />
          <p className="text-xs opacity-60 mt-2 text-center" style={{ color: "var(--ink)" }}>Scan with any UPI app · Amount ₹{order.total} is pre-filled</p>
        </div>
      )}

      {method === "bank" && (
        <div>
          <CopyField label="Account Name" value={BUSINESS.bank.accountName} />
          <CopyField label="Account Number" value={BUSINESS.bank.accountNumber} />
          <CopyField label="IFSC" value={BUSINESS.bank.ifsc} />
          <CopyField label="Bank" value={BUSINESS.bank.bankName} />
          <CopyField label="Amount" value={"₹" + order.total} />
        </div>
      )}

      <div className="mt-4 pt-3" style={{ borderTop: "1px solid var(--stone)" }}>
        <div className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--moss-dark)" }}>Upload Payment Receipt</div>
        {receipt ? (
          <div className="flex items-center gap-3">
            <img src={receipt} alt="Receipt preview" className="w-16 h-16 object-cover rounded-lg" style={{ border: "1px solid var(--stone)" }} />
            <button onClick={() => setReceipt(null)} className="text-xs font-medium" style={{ color: "var(--danger)" }}>Remove</button>
          </div>
        ) : (
          <label style={{ background: "var(--paper)", borderColor: "var(--stone)", color: "var(--ink)" }} className="block text-center rounded-xl border border-dashed p-4 text-sm cursor-pointer">
            {compressing ? "Processing…" : "Tap to upload a screenshot or photo"}
      ...
      <button
        ...
        onClick={place}
        style={{ background: receipt ? "var(--moss-dark)" : "var(--stone)" }}
        className="w-full mt-4 rounded-2xl py-3.5 text-white font-semibold text-sm"
      >
